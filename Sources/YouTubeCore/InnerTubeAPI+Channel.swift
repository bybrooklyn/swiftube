...
extension InnerTubeAPI {
    ...
    private func parseChannel(from json: [String: Any], channelId: String) throws -> (Channel, VideoGroup) {
        ...
        // Written as an if/else ladder rather than a `??` chain: `??` takes its
        // right-hand side as an autoclosure, and under Swift 6 an autoclosure
        // capturing `header` — a non-Sendable dictionary isolated to this actor —
        // is a "sending risks causing data races" error.
        var title = ""
        if let header {
            if let t = header["title"] as? String {
                title = t
            } else if let t = header["pageTitle"] as? String {
                title = t
            } else if let content = (header["content"] as? [String: Any])?["pageHeaderViewModel"] as? [String: Any] {
                // pageHeaderRenderer uses content.pageHeaderViewModel.title.content
                if let titleDict = content["title"] as? [String: Any], let t = extractText(titleDict) {
                    title = t
                } else if let t = content["title"] as? String {
                    title = t
                }
            }
        }
