...
public enum APIError: LocalizedError {
    ...
    case signInRequired
    /// Thrown when `/player` returned streaming data whose URLs are all
    /// cipher-protected (`signatureCipher` / `cipher`), which this client cannot
    /// decode directly.
    ///
    /// Distinct from `unavailable` because it says nothing about the video — the
    /// same video will usually play through a different InnerTube client. It
    /// exists so the caller can route to the fallback chain instead of showing
    /// the user an error, which is what the parse site always intended.
    ...
    public var errorDescription: String? {
        ...
        case .requiresDecryption:
            return "This video's streams need a different playback method."
        }
    }
}
