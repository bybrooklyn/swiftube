...
public actor InnerTubeAPI {
    ...
    /// Identity of the last satisfied path, so a genuine network change can be
    /// told apart from the routine churn described in `handlePathUpdate`.
    private var lastPathSignature: String? = nil
    ...
    private func handlePathUpdate(_ path: NWPath) {
        // Only reset visitorData when the network genuinely changed (VPN connect,
        // Wi-Fi switch), not merely because a path update arrived.
        //
        // Status alone is not enough to tell those apart on macOS. A Mac usually
        // has several interfaces up at once (Wi-Fi, Ethernet, Thunderbolt
        // bridges, VPN utuns), and NWPathMonitor reports satisfied → satisfied
        // repeatedly as their metadata changes — observed here firing every ~5
        // seconds on an idle machine. Clearing visitorData that often destroys
        // the session identity YouTube binds the PO token to, in the middle of
        // stream extraction. On iOS, with one interface, the same code is
        // effectively quiet, which is why it was not a problem upstream.
        //
        // So compare the path's actual identity and ignore updates that leave it
        // unchanged.
        ...
        guard prev == .satisfied, let previousSignature, previousSignature != signature else { return }

        visitorData = nil
        tubeLog.notice("visitorData cleared — network path changed (\(previousSignature, privacy: .public) → \(signature, privacy: .public))")
    }

    /// A stable description of which interfaces a path is actually using.
    nonisolated private static func signature(for path: NWPath) -> String {
        let interfaces = path.availableInterfaces
            .map { "\($0.name):\($0.type)" }
            .sorted()
            .joined(separator: ",")
        return "\(interfaces)|expensive=\(path.isExpensive)|constrained=\(path.isConstrained)"
    }
