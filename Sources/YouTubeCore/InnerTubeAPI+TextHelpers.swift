...
extension InnerTubeAPI {
    ...
    /// Returns the first of `keys` whose value in `dict` is a text renderer that
    /// `extractText` can read.
    ///
    /// InnerTube spells the same field differently across surfaces — `ownerText`
    /// vs `shortBylineText`, `viewCountText` vs `shortViewCountText`, `title` vs
    /// `headline` — so "try these keys in order" is the recurring shape.
    ///
    /// This exists instead of a `??` chain because `??` takes its right-hand side
    /// as an autoclosure, and an autoclosure capturing a non-Sendable
    /// `[String: Any]` isolated to this actor is a Swift 6 "sending risks causing
    /// data races" error. Passing the dictionary as a parameter has no such problem.
    func firstText(in dict: [String: Any], _ keys: String...) -> String? {
        for key in keys {
            if let nested = dict[key] as? [String: Any], let text = extractText(nested) {
                return text
            }
        }
        return nil
