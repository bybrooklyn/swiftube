...
extension InnerTubeAPI {
    ...
    func parseVideoRenderer(_ r: [String: Any]) -> Video? {
        guard let videoId = r["videoId"] as? String else { return nil }
        let title = firstText(in: r, "title", "headline") ?? ""
        let channelTitle = firstText(in: r, "ownerText", "shortBylineText") ?? ""
        ...
        var lengthText: String? = firstText(in: r, "lengthText")
        if lengthText == nil, let overlays = r["thumbnailOverlays"] as? [[String: Any]] {
            lengthText = overlays
        ...
        let viewCountText = firstText(in: r, "viewCountText", "shortViewCountText")
        var viewCount = viewCountText.flatMap { extractNumber($0) }
        // Further fallback: direct integer field (rare, but present in some compact API responses).
        if viewCount == nil { viewCount = r["viewCount"] as? Int }
        ...
        tubeLog.debug("videoRenderer id=\(videoId, privacy: .public) publishedTimeText='\(_ptv, privacy: .public)'")
    ...
    private func parsePlaylistVideoRenderer(_ r: [String: Any]) -> Video? {
        guard let videoId = r["videoId"] as? String else { return nil }
        let title = firstText(in: r, "title") ?? ""

        // channelTitle: shortBylineText preferred; ownerText as fallback
        let channelTitle = firstText(in: r, "shortBylineText", "ownerText") ?? ""
        ...
        let viewCountText = firstText(in: r, "shortViewCountText", "viewCountText")
        var viewCount = viewCountText.flatMap { extractNumber($0) }
        if viewCount == nil { viewCount = r["viewCount"] as? Int }
