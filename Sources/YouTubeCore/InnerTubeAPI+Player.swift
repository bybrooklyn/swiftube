...
extension InnerTubeAPI {
    ...
    private func parsePlayerInfo(from json: [String: Any], videoId: String) throws -> PlayerInfo {
        ...
        // Same reason as parseChannel's title ladder: a `??` autoclosure capturing
        // the actor-isolated `playabilityDict` is a Swift 6 data-race error.
        var playabilityReason = playabilityDict?["reason"] as? String
        if playabilityReason == nil, let errorScreen = playabilityDict?["errorScreen"] as? [String: Any] {
            if let subreason = (errorScreen["playerErrorMessageRenderer"] as? [String: Any])?["subreason"] as? [String: Any] {
        ...
        if !hasAnyURL {
            ...
            throw APIError.requiresDecryption
        }
