...
private func makeNonisolatedArtworkProvider(image: PlatformImage) -> (CGSize) -> PlatformImage {
    { _ in image }
}
...
extension PlaybackViewModel {
    ...
    /// AVAudioSession is iOS-only. macOS has no session to be interrupted by a
    /// phone call, and route changes are handled by the system, so this is a
    /// no-op here rather than a missing symbol at the call sites.
    #if !os(iOS)
    ...
    func updateNowPlayingInfo() {
        ...
        if let thumbURL = video.thumbnailURL {
            let snapshot: PlatformImage = cachedArtwork ?? PlatformImage()
