...
public final class PlaybackViewModel {
    ...
    public var error: Error? {
        didSet {
            ...
            DiagnosticsLogger.sendAutoPlaybackDiagnostic()
        }
    }
    ...
    public internal(set) var currentVideo: Video? = nil
    ...
    // intentional: the image is immutable after creation and the worst-case race is
    // that MediaPlayer gets nil on the first render (it will retry on the next state
    // update).
...
extension PlaybackViewModel: QualityEventHandler {
    ...
    private func checkWrongVideoOnFirstPlay() {
        ...
        DiagnosticsLogger.sendWrongVideoReport(
            intendedId: intended,
