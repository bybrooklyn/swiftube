...
extension PlaybackViewModel {
    ...
    public func load(video: Video) {
        ...
        DiagnosticsLogger.setVideoContext(id: video.id, title: video.title)
        ...
        if settings.historyState == .enabled, duration > 0 {
            ...
            // History is off (or there is no duration to report), so the returned
            // flush closure is deliberately dropped — this call is made only for
            // its side effect of rebinding the tracker's session to the new video.
    ...
    public func resume() {
        ...
        wasPlayingBeforeSuspend = false
        // Re-register remote commands (removed in suspend()).
        setupRemoteCommandCenter()
    ...
    func loadAsync(video: Video) async {
        ...
        // Seed the Now Playing widget BEFORE the ~10 s network phase so the title
        // and channel appear immediately.
        setupRemoteCommandCenter()
        #if os(iOS)
        // stop() calls setActive(false), which tears down the MediaRemote XPC
        // connection; re-activating here (rather than waiting until readyToPlay)
        // closes the gap during which PlayerRemoteXPC reports err=-12860/-12785
        // and the widget is absent.
        do {
            try AVAudioSession.sharedInstance().setActive(true)
        ...
        do {
            ...
            // Re-register remote commands before starting playback. Commands are
            // removed in suspend()/stop(); re-registering here ensures they work
            // when load() is called after a stop().
            setupRemoteCommandCenter()
            #if os(iOS)
            ...
            if let apiErr = error as? APIError {
                ...
                case .httpError(403):
                    shouldRetryWithFallback = hasAuthToken
                // Cipher-protected stream URLs say nothing about the video — a
                // different client almost always returns plain URLs for it. The
                // parse site throws this specifically so the fallback chain can
                // run, but nothing here routed it there, so a cold start (PO
                // token not yet minted) surfaced a dead end to the user instead
                // of retrying. Applies signed in or out.
